...
- (UICollectionViewLayoutAttributes *)layoutAttributesForSupplementaryViewOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (kind == UICollectionElementKindSectionHeader && [self.stickySections containsIndex:indexPath.section]) {
        ...
        switch (self.type) {
            case HRStickyHeaderFlowLayoutTypeReplace:
            {
            ...
            case HRStickyHeaderFlowLayoutTypeStack:
            {
                NSUInteger baseStickySection = [self.stickySections firstIndex];
                if (self.scrollDirection == UICollectionViewScrollDirectionVertical) {
                    frame.origin.y = MAX(contentOffset.y + (indexPath.section - baseStickySection) * CGRectGetHeight(frame), frame.origin.y);
                }
                else {
                    frame.origin.y = MAX(contentOffset.x + (indexPath.section - baseStickySection) * CGRectGetWidth(frame), frame.origin.x);
                }
                break;
            }
            case HRStickyHeaderFlowLayoutTypeShowOnBrowsMore:
            {
                if (self.scrollDirection == UICollectionViewScrollDirectionVertical) {
...
#pragma mark Pricate Methods Setters

- (void)setType:(HRStickyHeaderFlowLayoutType)type
{
    _type = type;
    if (type == HRStickyHeaderFlowLayoutTypeShowOnBrowsMore) {
        _stickySections = [NSIndexSet indexSetWithIndex:0];
    }
}

- (void)setStickySections:(NSIndexSet *)stickySections
{
    if (_type != HRStickyHeaderFlowLayoutTypeShowOnBrowsMore) {
